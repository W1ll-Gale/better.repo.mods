    ...
    class ItemUpgradePlayerTumbleLaunchPatch
    {
        static bool Prefix(ItemUpgradePlayerTumbleLaunch __instance)
        {
            ...
            foreach (var player in players)
            {
                ...
                TeamBoostersBase.Log.LogInfo($"Upgraded Player Tumble Launch for player {SemiFunc.PlayerGetSteamID(player)}");
            }

            TeamBoostersBase.Log.LogInfo("Applied Player Tumble Launch upgrade to all players.");
