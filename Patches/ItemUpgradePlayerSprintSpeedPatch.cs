    ...
    internal class ItemUpgradePlayerSprintSpeedPatch
    {
        static bool Prefix(ItemUpgradePlayerSprintSpeed __instance)
        {
            ...
            foreach (var player in players)
            {
                ...
                TeamBoostersBase.Log.LogInfo($"Upgraded Player Sprint Speed for player {SemiFunc.PlayerGetSteamID(player)}");
            }

            TeamBoostersBase.Log.LogInfo("Applied Player Sprint Speed upgrade to all players.");
